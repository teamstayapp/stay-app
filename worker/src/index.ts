...
async function analyzeImage(req: Request, env: Env, body: Record<string, unknown>): Promise<Response> {
  ...
  const scene = sceneResult.scene
  const fallbackModel = ALLOWED_MODELS.has(env.VENICE_MODEL || '') ? env.VENICE_MODEL! : MODEL
  const selectedModel = ALLOWED_VISION_MODELS.has(scene.visionModel)
    ? scene.visionModel
    : VISION_MODEL
...
const PROFESSION_LOOK: Record<string, string> = {
  ...
  schoolgirl: 'adult 25+ in a schoolgirl-style costume, clearly adult',
...
const PROFESSION_CHAT: Record<string, string> = {
  ...
  schoolgirl: 'Du er voksen 25+ i skolepige-kostume. Fræk, ikke barnlig.',
  milf: 'Du er voksen MILF. Erfaren og liderlig.',
  nurse: 'Du er en voksen sygeplejerske i fiktion. Ingen rigtig behandling.',
  teacher: 'Du er voksen lærer. Eleven er 25+. Stil frække opgaver. Kostume ok.',
...
function buildSystemPrompt(
  ...
  const equipmentIds = Array.isArray(profile.equipment)
    ? profile.equipment.filter((v): v is string => typeof v === 'string')
    : Array.isArray(profile.equipmentEntries)
      ...
      : []
  const hasStrap = equipmentIds.includes('strap_on')
  const anatomy = figure === 'master'
    ? `Du er mand. Du har pik. Sig ALDRIG min fisse. Brugerens krop: ${userAnatomyLabel} (sig din ${userAnatomyLabel}).`
    : hasStrap
      ? `Du er kvinde. Du har fisse og bryster. Ingen rigtig pik. Strap-on er valgt: sig min strap-on eller min sele-pik, aldrig at du har en rigtig pik. Brugerens krop: din ${userAnatomyLabel}.`
      : `Du er kvinde. Du har fisse og bryster. Sig ALDRIG min pik. Tal om min fisse, mine bryster, min røv. Brugerens krop: din ${userAnatomyLabel}.`
  ...
  return [
    ...
    profile.nsfw === true
      ? [
          plainText(profile.spicyLexicon, '') || 'Spicy dansk ordliste — brug de frække ord, ikke kliniske:',
          plainText(profile.spicyMinus, '') ? `Brug ALDRIG disse ord: ${plainText(profile.spicyMinus, '', 600)}.` : 'Undgå kuk, mule, flænse.',
          ...
          'Forbudte eller klodsede ord: kuk, mule, flænse, flænser, lem, skede, skidt, tissemand, tissekone, kønslæber, sædleder. Brug i stedet pik, fisse, røv, slikke, sprøjt, komme, knibe, suge.',
          'Ingen stjerne-*handlinger* og ingen citationstegn om replikker. Skriv som almindelig chat.',
          'Vær liderlig og konkret. Støn og kommandoer. Brug KUN de kropsdele du og brugeren faktisk har ifølge anatomi-reglerne nedenfor.',
        ].join(' ')
    ...
    Number.isFinite(Number(profile.partnerAge)) ? `Partneren er ${Math.max(18, Math.min(80, Number(profile.partnerAge)))} år. Tydeligt voksen.` : 'Partneren er tydeligt voksen, 18+.',
    ...
    (profile.fetishes as string[] | undefined)?.includes?.('sissy')
      ? [
          'Sissy-leg er slået til. Voksen. Aldrig barnligt sprog.',
          'Giv konkrete tøj-ordrer med det valgte lingeri. Nævn tingene ved navn.',
          'Eksempler: Tag trusserne på. Strømperne op. Korsettet strammes. Paryk på.',
          'Ros når tøjet sidder. Edge pikken i trusserne hvis brugeren har pik.',
        ].join(' ')
      : '',
    plainText(profile.memoryNotes, '') ? `Du kender brugeren. Husk dette og brug det naturligt, uden at recitere listen: ${plainText(profile.memoryNotes, '', 600)}.` : '',
    plainText(profile.lastMemory, '') ? `Sidste scene, kort: ${plainText(profile.lastMemory, '', 400)}. Referér kun hvis det passer.` : '',

    (profile.fetishes as string[] | undefined)?.includes?.('brat')
      ? 'Brat-leg: brugeren er fræk i munden. Du tæmmer med ordrer og edge. Ingen slå-how-to.'
      : '',
    (profile.fetishes as string[] | undefined)?.includes?.('protocol')
      ? 'Protocol: kræv knæ, titel (Mistress/Master) og at de venter. Få ord.'
      : '',
    (profile.fetishes as string[] | undefined)?.includes?.('worship')
      ? 'Worship: fødder, røv, bryster. De slikker og kysser der du peger.'
      : '',
    safe(profile.cockPreset, 'none') === 'bbc'
      ? 'Partneren er en voksen mørk mand med tyk pik. Beskriv krop, ikke race play. Ingen slurs.'
      : safe(profile.cockPreset, 'none') === 'bwc'
        ? 'Partneren er en voksen lys mand med tyk pik. Beskriv krop, ikke race play. Ingen slurs.'
        : '',
    customWish
