...
export default {
    async fetch(req, env) {
        ...
        if (req.method === 'POST' && (url.pathname === '/push/subscribe' || url.pathname === '/push/unsubscribe')) {
            ...
            if (!body)
                return json(req, env, { error: 'Ugyldig JSON' }, 400);
        ...
        if (!body)
            return json(req, env, { error: 'Ugyldig JSON' }, 400);
        if (url.pathname === '/image/generate')
            return generateImage(req, env, body);
        if (url.pathname === '/image/pose')
            return generatePartnerPose(req, env, body);
        if (url.pathname === '/vision')
            return analyzeImage(req, env, body);
        ...
        if (!reply)
            return json(req, env, { error: 'Venice svarede uden tekst' }, 502);
        const recorded = await recordUsage(env, usageGate.gate, selectedModel, data?.usage);
        if (!recorded)
            return json(req, env, { error: 'AI svarede, men forbruget kunne ikke registreres. Prøv igen.' }, 503);
...
async function analyzeImage(req, env, body) {
    ...
    if (imageDataUrl.length > 5_500_000)
        return json(req, env, { error: 'Billedet er for stort' }, 413);
    const sceneResult = await loadScene(req, env, safe(body.sceneId, 'soft-care'));
    if ('error' in sceneResult)
        return json(req, env, { error: sceneResult.error }, sceneResult.status);
    const scene = sceneResult.scene;
    ...
    const reply = data?.choices?.[0]?.message?.content?.trim();
    if (!reply)
        return json(req, env, { error: 'Venice svarede uden en billedanalyse' }, 502);
    const recorded = await recordUsage(env, usageGate.gate, selectedModel, data?.usage);
    if (!recorded)
        return json(req, env, { error: 'Billedet blev analyseret, men forbruget kunne ikke registreres. Prøv igen.' }, 503);
...
function buildPartnerPosePrompt(profile, scene) {
    const poses = [
...
function profileForPlan(value, plan) {
    ...
    if (plan === 'free') {
        profile.nsfw = false;
        if (profile.look === 'nsfw')
            profile.look = 'clothed';
    }
    return profile;
}
...
async function checkUsage(req, env, kind) {
    if (!env.FIREBASE_PROJECT_ID)
        return { error: 'Firebase-forbrugstælling er ikke konfigureret.', status: 503 };
    const authorization = req.headers.get('authorization');
    if (!authorization?.startsWith('Bearer '))
        return { error: 'Log ind igen for at bruge AI.', status: 401 };
    const token = authorization.slice(7);
    const identity = firebaseIdentity(token);
    if (!identity)
        return { error: 'Din login-session er ugyldig. Log ind igen.', status: 401 };
    const now = new Date();
    ...
    if (denied)
        return { error: 'Din login-session har ikke adgang til forbrugsdata.', status: 401 };
    ...
    if (unavailable)
        return { error: 'Forbrugsdata kunne ikke hentes. Prøv igen om lidt.', status: 503 };
...
async function subscribePush(req, env, body) {
    ...
    if (!identity)
        return json(req, env, { error: 'Log ind igen for at aktivere Web Push.' }, 401);
    ...
    if (!validPushEndpoint(endpoint))
        return json(req, env, { error: 'Ugyldigt Web Push-abonnement.' }, 400);
...
async function unsubscribePush(req, env, body) {
    if (!env.PUSH_SUBS)
        return json(req, env, { ok: true });
    const identity = await authenticatePushRequest(req, env);
    if (!identity)
        return json(req, env, { error: 'Log ind igen for at ændre Web Push.' }, 401);
    const endpoint = plainText(body.endpoint, '', 2_000);
    if (validPushEndpoint(endpoint))
        await env.PUSH_SUBS.delete(await pushStorageKey(identity.uid, endpoint));
    return json(req, env, { ok: true });
}
async function authenticatePushRequest(req, env) {
    if (!env.FIREBASE_PROJECT_ID)
        return null;
    const authorization = req.headers.get('authorization') || '';
    if (!authorization.startsWith('Bearer '))
...
async function runPushSchedule(env) {
    ...
    for (let page = 0; page < 10; page += 1) {
        ...
        for (const key of listed.keys)
            await processPushSubscription(env, key.name);
        if (listed.list_complete || !listed.cursor)
            break;
        cursor = listed.cursor;
    }
}
async function processPushSubscription(env, key) {
    const raw = await env.PUSH_SUBS?.get(key);
    if (!raw || !env.PUSH_SUBS)
        return;
    let row;
    try {
        row = JSON.parse(raw);
...
async function createVapidAuthorization(endpoint, env) {
    ...
    if (publicBytes.length !== 65 || publicBytes[0] !== 4)
        throw new Error('Ugyldig VAPID public key');
    const privateValue = (env.VAPID_PRIVATE || '').trim();
    if (!privateValue)
        throw new Error('VAPID private key mangler');
...
async function pushStorageKey(uid, endpoint) {
...
function nextPushDelay(intervalMin, mode) {
...
async function firestoreRead(env, token, path) {
    if (!env.FIREBASE_PROJECT_ID)
        return { status: 503 };
    try {
        let response = await fetch(`https://firestore.googleapis.com/v1/projects/${encodeURIComponent(env.FIREBASE_PROJECT_ID)}/databases/(default)/documents/${path}`, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(10_000) });
...
async function loadScene(req, env, sceneId) {
    ...
    let response;
    try {
        response = await fetch(`https://firestore.googleapis.com/v1/projects/${project}/databases/(default)/documents/scenePresets/${documentId}`, { headers: { Authorization: authorization }, signal: AbortSignal.timeout(10_000) });
    }
    catch {
    ...
    if (!response.ok)
        return { error: 'Scenen er ikke udgivet endnu.', status: 503 };
...
async function loadEquipmentCatalog(env, token) {
    ...
    if (result.status !== 200 || !Array.isArray(values))
        return null;
    return values.flatMap((value) => {
        ...
        const minimumPlan = minimumPlanValue === 'free' || minimumPlanValue === 'solo'
            ? minimumPlanValue
            : 'plus';
        if (!id || !prompt)
            return [];
...
function buildSystemPrompt(profileValue, stateValue, scene, intent, equipmentCatalog, touchZone = '') {
    ...
    const selectedEquipmentIds = Array.isArray(profile.equipment)
        ? profile.equipment.filter((value) => typeof value === 'string')
    ...
    const fetishIds = Array.isArray(profile.fetishes)
        ? profile.fetishes.filter((v) => typeof v === 'string').slice(0, 24)
        : [];
    const fetishes = Array.isArray(fetishValues)
        ? fetishValues.filter((v) => typeof v === 'string').slice(0, 12).join(', ')
        : 'edge, power';
    const plan = profile.plan === 'plus' ? 'plus' : profile.plan === 'solo' ? 'solo' : 'free';
    const planRank = { free: 0, solo: 1, plus: 2 };
    ...
    const equipment = equipmentEntries
        ...
        if (catalogById && (!catalogItem || !catalogItem.enabled || planRank[plan] < planRank[catalogItem.minimumPlan]))
            return [];
        if (!catalogById && allowedEquipment && !allowedEquipment.has(id))
            return [];
...
function veniceError(data, status) {
    ...
    if (data?.error?.message)
        return data.error.message.slice(0, 200);
